// This is the newly re-written QR Hook JS file
...
function QRHook(typeNumber, errorCorrectLevel){
...
QRHook.prototype = {
	...
	make:function(){
		if (this.typeNumber < 1){
			...
			for (typeNumber = 1; typeNumber < 40; typeNumber++) {
				rsBlocks = QRRSBlock.getRSBlocks(typeNumber, this.errorCorrectLevel);
	...
	makeImpl : function(test, maskPattern) {
		...
		if (this.dataCache == null){
			this.dataCache = QRHook.createData(this.typeNumber, this.errorCorrectLevel, this.dataList);
		}
	...
	createMovieClip:function(target_mc, instance_name, depth){
		...
		for (row = 0; row < this.modules.length; row++){
			y = row * cs;
			
			for (col = 0; col < this.modules[row].length; col++){
				x = col * cs;
	...
	mapData:function(data, maskPattern){
		...
		for (col = this.moduleCount - 1; col > 0; col -= 2) {
			...
			// This seems like a lame function
			while(true){
				for (c = 0; c < 2; c++) {
...
QRHook.createData = function(typeNumber, errorCorrectLevel, dataList) {
	...
	while (true) {
		...
		buffer.put(QRHook.PAD0, 8);
		
		if (buffer.getLengthInBits() >= totalDataCount * 8) {
			break;
		}
		buffer.put(QRHook.PAD1, 8);
	}

	return QRHook.createBytes(buffer, rsBlocks);
};
...
QRHook.createBytes = function(buffer, rsBlocks) {
	...
	for (var r = 0; r < rsBlocks.length; r++){
		...
		for (var p = 0; p < dcdata[r].length; p++) {
			dcdata[r][p] = 0xff & buffer.buffer[p + offset];
		}
		offset += dcCount;
...
(function($){
	$.fn.qrHook = function(options){
		...
		var createDiv = function(){
			...
			var moduleCount = qrHook.getModuleCount();
			...
			for(row = 0; row < moduleCount; row++){
				...
				for(col = 0; col < moduleCount; col++){
					outputString += "<div class='" + (qrHook.isDark(row, col) ? 'qrBG1' : 'qrBG2') + "'></div>";
				}
				outputString += "</div>";
			}
